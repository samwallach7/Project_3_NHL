...
For this section of the project, we were interested in creating a webpage to supply a variety of NHL data including player statistics, draft selections and team geoplotting. The user can select a page from the top navigation bar and interact with each page in the following manner:

Player Info
-  Select a player from the dropdown list and click submit to view their general information and season statistics. To change player, select a different name from the dropdown list and then click submit.
...
Draft:
- Select a year from the dropdown list to view NHL Draft statistics such as the average age by nationality, the player count by nationality and the amount of players drafted per team for that year.
...
-  We chose to utilize data from a Kaggle Professional Hockey Database and a SportsDataIO API integration. This data was used as it appeared without any other manipulation. The set of files that tracked a variety of NHL statistics included: team performance, player scoring, and NHL Draft results. Eliminating any possible biases and showcasing the diversity of the player pool by countries represented was an important consideration when putting together the NHL Draft page. On the NHL Map, we felt an importance in plotting all of the professional hockey teams at the higher levels of the sport and not just the NHL teams. This helps to show the access that fans in varying markets in the U.S. and Canada have to the professional game.
-  Kaggle: Professional Hockey Database (https://www.kaggle.com/datasets/open-source-sports/professional-hockey-database?resource=download)
-  SportsDataIO: NHL API Documentation (https://sportsdata.io/developers/api-documentation/nhl#)

Database Design
-  We utilized ETL workflows to pull the data from its original source, transform the data if necessary, and the save the datasets as tables in a SQL database. We then pulled the data in from this database for use on the various pages. We chose to use a SQL database for the ease in working with csv files, which was how the majority of our data was presented. The project's ERD is available within this repository.
